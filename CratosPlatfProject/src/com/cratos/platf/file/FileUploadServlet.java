...
public class FileUploadServlet extends HttpServlet {
    ...
    protected static Font FONT = new Font("微软雅黑", Font.ITALIC, 12);

    protected static final Color COLOR = Color.WHITE;

    protected static final Color FONT_COLOR = new Color(255, 255, 255, 150);

    protected static final Color FONT_SHADOW_COLOR = new Color(170, 170, 170, 77);
    ...
    @HttpMapping(url = "/upload/face", auth = false, comment = "上传头像 以正方形规格存储") // 
    public void uploadFace(HttpRequest req, HttpResponse resp) throws IOException {
    ...
    @HttpMapping(url = "/upload/chatfile", auth = false, comment = "上传文件") // 
    public void uploadChatFile(HttpRequest req, HttpResponse resp) throws IOException {
        uploadImg(req, resp, "chatfile", Utility.uuid(), null, null, 10 * 1024 * 1024L);
    }

    @HttpMapping(url = "/upload/", auth = false, comment = "上传文件") // 
    public void uploadXXX(HttpRequest req, HttpResponse resp) throws IOException {
        uploadImg(req, resp, req.getRequstURILastPath(), Utility.uuid(), null, null, 10 * 1024 * 1024L);
    }
    ...
    protected void uploadImg(HttpRequest req, HttpResponse resp, int[] widths, final ImageRatio ratio, long max) throws IOException {
        uploadImg(req, resp, req.getRequstURILastPath(), null, widths, ratio, max);
    }
    ...
    protected void uploadImg(HttpRequest req, HttpResponse resp, String dir, int[] widths, final ImageRatio ratio, long max) throws IOException {
        uploadImg(req, resp, dir, null, widths, ratio, max);
    }
    ...
    protected void uploadImg(HttpRequest req, HttpResponse resp, String dir, String fileid0, int[] widths, final ImageRatio ratio, long max) throws IOException {
        uploadImg(req, resp, dir, fileid0, widths, ratio, max, null);
    }

    protected void uploadImg(final HttpRequest req, HttpResponse resp, String dir, String fileid0, int[] widths, final ImageRatio ratio, final long max, Runnable runner) throws IOException {
        for (MultiPart part : req.multiParts()) {
            final String mime = MimeType.getByFilename(part.getFilename());
            if (!mime.contains("image/")) { //不是图片
                uploadBin(req, resp, dir, fileid0, max);
                return;
            }
            String fileid = service.storeMultiJPGFile(dir, fileid0, widths, ratio, part.getContentBytes(max), runner);
            if (fileid.isEmpty()) {
                resp.finishJson(RetCodes.retResult(RetCodes.RET_UPLOAD_FILETOOBIG));
            } else {
                resp.finish("{\"success\":true,\"retcode\":0,\"fileid\":\"" + fileid + "\",\"filename\":\"" + part.getFilename() + "\",\"filelength\":" + part.getReceived() + "}");
            }
            return;
        }
        resp.finishJson(RetCodes.retResult(RetCodes.RET_UPLOAD_NOFILE));
    }
