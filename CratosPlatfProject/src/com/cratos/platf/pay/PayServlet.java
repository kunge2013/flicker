...
public class PayServlet extends HttpServlet {
    ...
    public void checkPay(HttpRequest req, HttpResponse resp) throws IOException {
        if (info) logger.info("pay.check.request = " + req);
    ...
    public static PayNotifyRequest unionNotifyRequest(Logger logger, HttpRequest req) throws IOException {
        logger.info("unionpay.notify.request = " + req);
    ...
    public static PayNotifyRequest weixinNotifyRequest(Logger logger, HttpRequest req) throws IOException {
        logger.info("weixinpay.notify.request = " + req);
    ...
    public static PayNotifyRequest alipayNotifyRequest(PayService service, Logger logger, HttpRequest req) throws IOException {
        logger.info("alipay.notify.request = " + req);
        final TreeMap<String, String> map = new TreeMap<>();
...
//
//    @HttpMapping(url = "/pay/nowpay/notify", auth = false, comment = "现在支付异步通知")
//    public void nowpaynotify(HttpRequest req, HttpResponse resp) throws IOException {
//        logger.info("nowpay.notify.request = " + req);
//        final Map<String, String> map = NowPayService.parseToMap(req.getBodyUTF8());
//        List<String> emptyKeys = new ArrayList<>();
//        for (Map.Entry<String, String> en : map.entrySet()) { //去掉空值的参数
//            if (en.getValue().isEmpty()) emptyKeys.add(en.getKey());
//        }
//        emptyKeys.forEach(x -> map.remove(x));
//        resp.finish(service.notify(new PayNotifyRequest(NowPayService.PAYTYPE_NOWPAY, map)).getRetinfo());
//    }
//
//    @HttpMapping(url = "/pay/jxpay/notify", auth = false, comment = "金鑫支付异步通知")
//    public void jxpaynotify(HttpRequest req, HttpResponse resp) throws IOException {
//        logger.info("jxpay.notify.request = " + req);
//        final Map<String, String> map = JxPayService.parseToMap(req.getBodyUTF8());
//        resp.finish(service.notify(new PayNotifyRequest(JxPayService.PAYTYPE_JXPAY, map)).getRetinfo());
//    }

    @HttpMapping(url = "/pay/zfpay/notify", auth = false, comment = "融易通支付异步通知")
    public void zfpaynotify(HttpRequest req, HttpResponse resp) throws IOException {
        logger.info("zfpay.notify.request = " + req);
        final Map<String, String> map = ZfPayService.parseToMap(req.getBodyUTF8());
        resp.finish(service.notify(new PayNotifyRequest(ZfPayService.PAYTYPE_ZFPAY, map)).getRetinfo());
    }

    @HttpMapping(url = "/pay/zxpay/notify", auth = false, comment = "众鑫易付异步通知")
    public void zxpaynotify(HttpRequest req, HttpResponse resp) throws IOException {
        logger.info("zxpay.notify.request = " + req);
        final Map<String, String> map = ZxPayService.parseToMap(req.getBodyUTF8());
        resp.finish(service.notify(new PayNotifyRequest(ZxPayService.PAYTYPE_ZXPAY, map)).getRetinfo());
    }

    @HttpMapping(url = "/pay/neweasypay/notify", auth = false, comment = "新融易付异步通知")
    public void neweasypay(HttpRequest req, HttpResponse resp) throws IOException {
        logger.info("neweasypay.notify.request = " + req);
        final Map<String, String> map = NewEasyPayService.parseToMap(req.getBodyUTF8());
        resp.finish(service.notify(new PayNotifyRequest(NewEasyPayService.PAYTYPE_NEWEASYPAY, map)).getRetinfo());
    }
}
